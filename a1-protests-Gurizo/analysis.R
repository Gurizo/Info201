...
# Open the dataframe by clicking the spreaColumndsheet icon in the Environment or by
...
# Column1: Date, Column2: Location, Column3: Attendees, Column4: Event legacy see tags, Column5: tags, Column6: curated, Column7: source, Column8: total articles
...
# `num_features`
num_features <- length(names(protest_data))
num_features
...
num_in_wa <- sum(str_detect(locations, "WA", negate = FALSE))
num_in_wa
...
count_protests_in_location <- function(location) {
  location_data <- str_detect(locations, location)
  num_protests_location <- sum(location_data)
  
  if (num_protests_location == 0) {
    return(paste0("Sorry, that location is not found."))
  } else {
    return(paste0("There were ", num_protests_location, " protests in ", location, "."))
  }
}
...
time_span <- as.numeric(difftime(most_recent_protest, earliest_protest, units = "days"))
time_span
...
high_level_purposes <- sub("\\(.*\\)", "", purposes)
trimws(high_level_purposes)
...
sorted_table <- sort(high_level_table, decreasing = TRUE)
View(sorted_table)
category_names <- names(sorted_table)
category_names[1]
category_names[4]
