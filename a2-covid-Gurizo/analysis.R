...
highest_cases_in_each_state <- counties %>%
  ...
  distinct() %>%
  group_by(state) %>%
  filter(row_number(desc(cases)) == 1) %>%
...
all_totals <- inner_join(national, total_cases_counties, by = "date")
...
national_county_diff <- filter(all_totals, county_total_cases != cases)
...
-------------------------------------------------
  ...
  # Which year has the most number of COVID? 
  # What was the month when the most new cases occurred?

  #  Write code (at least 2-3 lines) that will answer your question
  # Assuming the date column in the national dataframe is in the format "YYYY-MM-DD"
  national %>%
  ...
  slice(1) 
  # 2022 with 31760084178 cases.

  monthly_cases <- national %>%
  mutate(month = lubridate::month(date)) %>%
  group_by(month) %>%
  summarize(new_cases = sum(cases))
  
  most_cases_month <- monthly_cases %>%
  filter(new_cases == max(new_cases)) %>%
  pull(month)
  most_cases_month_name <- month.name[most_cases_month]
  most_cases_month_name
  # January had the most COVID occurred.
